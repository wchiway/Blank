...
export async function checkUpdate(): Promise<AppVersion | undefined> {
  ...
  let url = 'https://github.com/wchiway/Blank/releases/latest/download/latest.yml'
  if (updateChannel == 'beta') {
    url = 'https://github.com/wchiway/Blank/releases/download/pre-release/latest.yml'
  }
...
export async function downloadAndInstallUpdate(version: string): Promise<void> {
  ...
  const baseUrl = `https://github.com/wchiway/Blank/releases/download/${releaseTag}/`
  const fileMap: Record<string, string> = {
    'win32-x64': `blank-windows-${version}-x64-setup.exe`,
    'win32-arm64': `blank-windows-${version}-arm64-setup.exe`,
    'darwin-x64': `blank-macos-${version}-x64.pkg`,
    'darwin-arm64': `blank-macos-${version}-arm64.pkg`
  }
  ...
  const apiUrl = `https://api.github.com/repos/wchiway/Blank/releases/tags/${releaseTag}`
